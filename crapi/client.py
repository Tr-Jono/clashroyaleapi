...
from crapi.player import Player
...
class Client:
    ...
    def _get_methods_base(self, endpoint, keys=None, exclude=None, max_results=None, page=None, **kwargs):
        ...
        if not isinstance(keys, collections.Iterable) and isinstance(keys, str) and keys != "":
            keys = (keys,)
        if not isinstance(exclude, collections.Iterable) and isinstance(exclude, str) and exclude != "":
            exclude = (exclude,)
        ...
        for param in keys, exclude, kwargs:
            if param is None:
                ...
                if not all([(isinstance(elem, str) and elem != "") for elem in param]):
    ...
    def get_player(self, player_tag, keys=None, exclude=None, max_results=None, page=None):
        return Player.de_json(self._get_methods_base(f"player/{player_tag}", keys, exclude, max_results, page))
